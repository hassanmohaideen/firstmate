...
- **Optional self-hosted Discord** - run a private owner/guild/channel-bound bot on an always-on Mac so the captain can message this local Firstmate through Discord directly, with no myfirstmate.io dependency or Relay quota; it stays inert until locally configured and uses the same durable supervision queue rather than a second control plane.
...
- Git and the GitHub CLI; authenticate through `gh auth login` before dispatching work that pushes to GitHub or performs an authenticated GitHub read.
...
Run `gh auth login` before GitHub-needing work; local-only ships and plain scouts remain available while signed out.
...
Optional secondmates extend this to persistent local or whole-home remote second mates, dispatch profiles let you steer which harness handles which task, opt-in Relay lets the same fleet answer public mentions, and the separate self-hosted Discord path can connect one local home directly to a captain-owned bot.
`codex-app` is not a runtime backend yet; [docs/codex-app-backend.md](docs/codex-app-backend.md) owns the Codex App boundary.

Full architecture - the supervision engine, worktree isolation, secondmates, dispatch profiles, project modes, optional Relay and direct Discord transports, fleet sync, and self-update - is in [docs/architecture.md](docs/architecture.md).
...
- [docs/configuration.md](docs/configuration.md) - environment variables, `FM_HOME`, runtime backend selection, optional Relay and direct Discord configuration, the files you set, and harness support.
- [docs/discord-bot.md](docs/discord-bot.md) - configure, authorize, run, secure, and troubleshoot the optional direct self-hosted Discord bot on macOS.
