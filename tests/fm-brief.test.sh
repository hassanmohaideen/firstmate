...
test_rtk_runtime_opt_in_is_removed() {
  local home status=0
  home="$TMP_ROOT/rtk-removed-home"
  mkdir -p "$home/data"

  FM_HOME="$home" "$ROOT/bin/fm-brief.sh" rtk-ship firstmate --mode no-mistakes --rtk-compact \
    >"$TMP_ROOT/rtk.out" 2>"$TMP_ROOT/rtk.err" || status=$?
  expect_code 1 "$status" "removed --rtk-compact flag was accepted"
  assert_grep "RTK v1 is verification-only" "$TMP_ROOT/rtk.err" \
    "removed runtime opt-in did not explain the verification-only contract"
  assert_absent "$home/data/rtk-ship/brief.md" \
    "removed runtime opt-in still scaffolded a brief"

  status=0
  FM_HOME="$home" "$ROOT/bin/fm-brief.sh" rtk-valued firstmate --mode no-mistakes --rtk-compact=1 \
    >/dev/null 2>&1 || status=$?
  expect_code 1 "$status" "valued removed runtime opt-in was accepted"
  assert_absent "$home/data/rtk-valued/brief.md" \
    "valued removed runtime opt-in still scaffolded a brief"
  pass "fm-brief.sh: RTK runtime brief exposure is removed"
}
