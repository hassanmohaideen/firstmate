...
The inline rules in `AGENTS.md` section 3 still bind: detect, then consent, then install - never install anything the captain has not approved in this session - and no work is dispatched until the tools it needs are present.
GitHub auth is a per-dispatch requirement, not a universal one: only GitHub-needing work waits on it (see `NEEDS_GH_AUTH` below), while non-GitHub work such as a local-only ship or a plain scout dispatches even while signed out.
...
- `NEEDS_GH_AUTH` - no GitHub session is available, which gates only GitHub-needing dispatch (a ship pushing to GitHub, an authenticated GitHub read) while never blocking non-GitHub work such as a local-only ship or a plain scout, and treats a dispatch whose GitHub need is uncertain as GitHub-needing (`fm_github_dispatch_requires_auth` in `bin/fm-github-context-lib.sh` owns that decision, enforced by the spawn gate).
  So keep dispatching non-GitHub work, and for GitHub-needing work ask the captain to run `! gh auth login` (interactive; you cannot run it for them).
  From the session-start deferred network stage this line is also how an unreachable network shows up: `gh` cannot validate its token offline and reports the same failure. Confirm reachability before asking the captain to re-authenticate a credential that may be fine.
  The same line also arrives from a blocked `bin/fm-spawn.sh` or `bin/fm-promote.sh` launch when the selected GitHub destination's active account is signed out; that worker will not launch until the concrete destination verifies, so relay the exact login and re-verify rather than launching around it.
- `GITHUB_ACCESS_REQUIRED: <what access to which target>` - a `bin/fm-spawn.sh`/`bin/fm-promote.sh` launch proved the selected account lacks the exact write, read, or membership access it named; this is authorization, not a signed-out session, so escalate the concrete missing access on the named target and do not offer a login command that would not change it.
- `GH_AUTH_INDETERMINATE: <reason>` - the launch could not establish a concrete verdict for its selected destination (network or credential-store failure, a changed, mixed, missing, or malformed destination, an account-isolation gap, or a hidden 404).
  It is never a login prompt: resolve the named uncertainty, then relaunch so the same gate re-probes.
  Restore a repointed remote to the task's recorded destination before relaunch; relaunch refuses selection flags, so an intentional destination change needs a fresh task intake with `--github-host` rather than adoption by the existing task.
