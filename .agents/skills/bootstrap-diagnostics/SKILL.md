...
- `GH_AUTH_INDETERMINATE: <reason>` - the launch could not establish a concrete verdict for its selected destination (network or credential-store failure, a changed, mixed, missing, or malformed destination, an account-isolation gap, or a hidden 404). It is retryable and never a login prompt: resolve the named uncertainty - reachability, the repointed or ambiguous remote, or an authoritative re-selection with `--github-host` - then relaunch so the same gate re-probes.
