...
config/rtk      optional exact RTK pilot selection for explicitly opted-in ordinary tasks; LOCAL, gitignored, and not inherited; see docs/configuration.md "Optional RTK output compaction"
...
When an ordinary ship or scout is explicitly opted into supplemental RTK orientation, scaffold with `--rtk-compact`; the generated contract and `bin/fm-rtk.sh` own the boundary, and charters refuse the flag.
