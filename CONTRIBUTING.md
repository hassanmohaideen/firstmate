...
`--portable` is the routine complete local regression and leaves real-Herdr integration to its required CI lane, while `--all` is the explicit operator path that includes it.
