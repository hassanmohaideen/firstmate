...
class LaneExecutor:
    ...
    def _grant_leased_checkout_access(self) -> None:
        ...
        # its files. Grant world read+traverse on non-symlink checkout entries,
        # including Git metadata needed by read-only Git commands, so every leased
        # identity can read the scripts while the tree stays unwritable by children.
