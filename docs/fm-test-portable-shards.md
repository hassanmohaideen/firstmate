...
Because each leased identity runs its script with the runner-owned checkout as the working directory, the executor grants world read and traverse on non-symlink checkout entries, including `.git` for required read-only Git commands, so any leased identity can read the selected script and the files it sources while the tree stays unwritable by tests. Checkout credential persistence is disabled in every leased-test job, so the runner token is never written into the checkout the leased identities can read.
