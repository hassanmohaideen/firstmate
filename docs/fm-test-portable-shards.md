...
Because each leased identity runs its script with the runner-owned checkout as the working directory, the executor grants world read and traverse on the checkout tree, read-only, so any leased identity can read the selected script and the files it sources while the tree stays unwritable by tests and a source checkout holds no secret material.
