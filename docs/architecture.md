...
GitHub launch verification is a separate pre-endpoint boundary owned entirely by `bin/fm-github-context-lib.sh`; spawn, relaunch, and scout promotion call that owner rather than carrying their own destination or authorization decisions.
The owner library documents the decision contract, while the driver headers own their exact flags and mutation order.
`fm-spawn.sh`'s header owns the exact worktree-refusal mechanics, while `tests/fm-spawn-pool-base-freshen.test.sh` owns the portable base-freshness regression coverage.
...
Crewmate and scout dispatch can stay on the static crewmate harness resolved by `config/crew-harness`, or it can use the layered dispatch profiles in home-local `config/crew-dispatch.json` and repository-owned `defaults/crew-dispatch.json`.
The dispatch layers are intentionally judgment-based: firstmate reads their natural-language rules at intake, chooses the best matching rule under the precedence owned by `configuration.md`, resolves profile arrays itself from current quota output under the `AGENTS.md` section 4 intake boundary and the `quota-array-dispatch` selection procedure, and passes only concrete `--harness`, `--model`, `--effort`, and (for a claude profile) `--advisor` axes to `fm-spawn.sh`.
...
When either file is effective, `fm-spawn.sh` refuses crewmate and scout launches without an explicit harness, so `config/crew-harness` is only automatic when no dispatch profile is active.
...
`config/crew-dispatch.json` is inherited too; synced secondmate repositories layer it with the same tracked dispatch default when spawning their own crewmates.
