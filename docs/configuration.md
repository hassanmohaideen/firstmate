...
The canonical `state/.discord-bot-service/reconnect.json` is a mode-`0600` record holding only an opaque bot-authentication fingerprint, per-home reconnect pressure, last-attempt time, Discord's `server_not_before` retry deadline with its `server_wait_ms` bounded outstanding wait, Discord session-start counters, and the private resumable session id, validated Gateway URL, sequence, and authenticated bot user id needed to preserve pacing, Resume, and intake across process replacement, clock changes, and filtering changes; it contains no bot token or configured owner, guild, or channel id.
The mode-`0600` `state/.discord-bot-service/reconnect-suppression.json` independently preserves a secret-safe server deadline or operator-intervention requirement when the primary reconnect record cannot be replaced, and `retry` or `configure` clears that fallback.
