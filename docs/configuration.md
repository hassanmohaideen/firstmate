...
Portable and real-Herdr shard evidence and coverage rules are in [fm-test-portable-shards.md](fm-test-portable-shards.md); [herdr-backend.md](herdr-backend.md#destructive-lab-safety) owns the real-Herdr isolation boundary, and [runtime-backends.md](verification/runtime-backends.md#herdr) owns active evidence.
...
When a home-local or tracked crew-dispatch profile is effective, crewmate and scout spawns require an explicit resolved harness instead of automatically falling back to `config/crew-harness`.
...
## Crew dispatch profiles (config/crew-dispatch.json / defaults/crew-dispatch.json)

`config/crew-dispatch.json` is an optional local, gitignored file containing natural-language rules that firstmate reads before dispatching a crewmate or scout.
An optional repository-owned tracked `defaults/crew-dispatch.json` supplies standing rules that take effect after clone or update without writing into the home's private configuration.
...
The universal toolchain is node, git, gh, no-mistakes v1.31.2 or newer, compatible gh-axi, chrome-devtools-axi, compatible lavish-axi, compatible tasks-axi per "Backlog backend" above, and compatible quota-axi.
GitHub auth via `gh auth login` is readiness for GitHub-needing dispatch, not part of universal dispatch readiness: it gates a ship pushing to GitHub or an authenticated GitHub read, while a local-only ship or plain scout remains dispatchable when signed out and an uncertain dispatch fails closed as GitHub-needing.
...
When a local or tracked crew-dispatch profile is effective, bootstrap also requires `jq` for dispatch profile validation.
