...
RTK v1 is a non-executing operator verifier, not a configuration or runtime feature. There is no `config/rtk` activation and no task, bootstrap, session-start, secondmate, worker-runtime, prompt, hook, plugin, filter, shell-startup, or `PATH` integration.

On Darwin arm64, `bin/fm-rtk.sh verify` inspects only `$FM_HOME/data/tools/rtk/v0.45.0/aarch64-apple-darwin/rtk`. The artifact must be a regular non-symlink executable with SHA-256 `17d00d61a533a442c61f1be49d8a9321225557f64021d5b70fd8eb75ed8fb0be`, which binds the reviewed `rtk 0.45.0` artifact without executing it. The verifier creates no files and runs no project or artifact command. Firstmate never downloads, installs, copies, stages, activates, discovers, or updates the artifact; placing or removing it is a separately authorized operator action.

[`rtk-output-compaction.md`](rtk-output-compaction.md) owns the path-safety boundary, operator procedure, update review, rollback policy, and graduation criterion. RTK is not part of the universal or backend-specific toolchain and must never become a bootstrap or dispatch requirement.
