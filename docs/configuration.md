...
The mode-`0600` `state/.discord-bot-service/reconnect-suppression.json` independently preserves the same secret-safe boot-bound server wait or an operator-intervention requirement when the primary reconnect record cannot be replaced, and `retry` or `configure` clears that fallback.
