...
Observed guarantees: a contended presentation lock refused the teardown before the isolated copy was returned, with the task branch, every durable record, and the endpoint intact and no pane close attempted; the retry after the contention cleared returned the copy, closed the pane under the lock, and removed the records; an unknown structured-presence result after an attempted projected close retained the journal and every record with a nonzero exit; every presence-gate mode accepted only a structured not-found as gone; and a missing adapter now reaches the same prerequisite refusal under stock Bash 3.2 rather than exiting its conditional before the caller can return nonzero.
