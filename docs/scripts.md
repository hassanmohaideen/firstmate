...
| `fm-herdr-ci-cleanup.sh` | Snapshot and tear down only job-owned `fm-lab-*` sessions in a Herdr CI shard        |
| `fm-test-run.sh`         | Select tests, compose lanes and manifests, check coverage, aggregate evidence, and render results |
| `fm-test-supervisor.py`  | Execute test manifests with credential-domain containment and durable schema-v2 evidence |
...
| `fm-no-mistakes-review.sh` | Guard review responses and readiness with a durable ledger and crash-recoverable single-owner audit |
